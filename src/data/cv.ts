...
export const bio: string[] = [
  ...
  'My main focus is **graph learning** for molecular representation and **ADMET prediction** — but I am also comfortable across broader AI-for-drug-discovery and bioinformatics problems, from **drug repurposing** to multi-omics target discovery (e.g., aging-related MASLD). I build **interpretable models** that use attention to reveal which molecular substructures drive each prediction, so the model’s reasoning stays transparent rather than opaque. Of my two ADMET manuscripts, one is under peer review and the other is being finalized for submission.',
