...
export const bio: string[] = [
  'Hi, I’m **Myungjin Lee**, a machine learning researcher focused on **AI for drug discovery**. I recently completed my M.S. in Computer Science and Artificial Intelligence at Dongguk University _(major: AI in Healthcare and Medicine)_, and earned my B.S. in Industrial & Management Engineering from Induk University as valedictorian. I’m now **looking for opportunities** where I can apply machine learning to drug discovery and molecular data.',
  'My main focus is **graph learning** for molecular representation and **ADMET prediction** — though I also have experience with broader AI-for-drug-discovery and bioinformatics problems, from **drug repurposing** to transcriptomic analysis for target discovery in aging-related MASLD. I build **interpretable AI models** that use an **attention mechanism** to reveal which molecular substructures drive each prediction, so the model’s reasoning stays transparent rather than opaque. Of my two ADMET manuscripts, one is under peer review and the other is being finalized for submission.',
  'Looking ahead, I want to keep advancing **drug discovery** with interpretable, graph-based AI — connecting molecular representation to **drug–target interaction** and **drug-response prediction**, so a single framework can reason from a molecule’s structure all the way to its effect on cells. Longer term, I’d love to extend this toward **generative models for molecular design**, turning what a model learns into new drug candidates.',
]
