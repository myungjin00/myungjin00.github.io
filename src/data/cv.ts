...
export const bio: string[] = [
  'Hi, I’m **Myungjin Lee**, a machine learning researcher focused on **AI for drug discovery**. I recently completed my **M.S. in Computer Science and Artificial Intelligence** at Dongguk University _(major: AI in Healthcare and Medicine)_, and earned my B.S. in Industrial & Management Engineering from Induk University as **valedictorian**. I’m now looking for industry roles where I can apply machine learning to drug discovery and molecular data.',
  'My main focus is **graph learning** for molecular representation and **ADMET prediction** — though I have also worked on broader AI-for-drug-discovery and bioinformatics problems, from **drug repurposing** to transcriptomic analysis for target discovery in aging-related MASLD. I build **interpretable models** that use attention to reveal which molecular substructures drive each prediction, so the model’s reasoning stays transparent rather than opaque. Of my two ADMET manuscripts, one is under peer review and the other is being finalized for submission.',
